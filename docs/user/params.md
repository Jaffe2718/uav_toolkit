...
- `model_checkpoint_path`:
    - str
    - 描述: 模型的检查点路径。模型检查点是一个文件，包含了模型的权重和其他参数。在运行模型之前，必须先加载模型的检查点。

- `model_checkpoint_type`:
    - str: ["vit_h", "vit_l", "vit_b"]
    - 描述: 模型的类型。目前支持三种模型：`vit_h`、`vit_l`和`vit_b`。这三种模型分别是Vision Transformer（ViT）的`h`、`l`和`b`版本。这三种模型的区别在于模型的大小和性能。`vit_h`是最小的模型，`vit_b`是最大的模型。

- `input_raster`:
    - str
    - 描述: 输入栅格数据的路径。栅格数据是一种用于存储地理空间数据的格式。在这个工具中，栅格数据通常是无人机拍摄的图像。
...
- `output_feature_class`:
    - str
    - 描述: 输出要素类的路径。要素类是一种用于存储地理空间数据的格式。在这个工具中，要素类通常是模型生成的掩膜。

- `use_gpu`:
    - bool
    - 描述: 是否使用GPU，如果设置为`True`，则使用GPU运行模型；如果设置为`False`，则使用CPU运行模型。当然，如果GPU不可用，那么即使设置为`True`，也会使用CPU运行模型。
...
![DJI_0644_boxes](images/params/DJI_0644-boxes.png)
...
![box0-seg](images/params/DJI_0644-box0-seg.png)
...
![box1-seg](images/params/DJI_0644-box1-seg.png)
...
![box2-seg](images/params/DJI_0644-box2-seg.png)
...
![box3-seg](images/params/DJI_0644-box3-seg.png)
...
![box4-seg](images/params/DJI_0644-box4-seg.png)
